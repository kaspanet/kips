...
interface KaspaProvider {
  request: (protocol:string,method:string,params:Object) => Promise<Data>;
...
interface Request {
  eventId: string, // A unique id that this request associates with. E.g. uuidv4
...
interface Response {
  eventId: string, // A unique id that this response associates with. E.g. uuidv4
  extensionId: string; // `browser.runtime.id`
  data: {
...
interface Notification { // Do not require a response, and therefore no id.
  data: {
    protocol: string | undefined;
    data: any;
  }, 
...
async function request(protocol:string,method:string,params:Object): Promise<unknown> {
  const requestEvent = new CustomEvent("kaspa:invoke", {
    detail: Object.freeze({
      eventId: uuidv4(),
      extensionId: "HardcodedExtensionIdEmittedByExtension", 
      data: {
        protocol,
        method,
        params
