...
### Micropayments
In the following section we discuss micropayments of extremely small values ($<< 0.1$ `KAS`). Despite seeming like a long-shot, we see such (future) payments as an important aspect of Kaspa and the economy we strive that would be built over it. We will assume that every user wallet holds at least 1 `KAS` (or $10^8$ dworks).
...
This exact behavior can be emulated also in the UTXO model by a mutually signed transaction. The buyer and the seller can mutually create a transaction with an input of, say, $10$ `KAS` from each and an output of $10.0001$ to the seller and $9.9999$ to the buyer, which results in negligible mass. 
...
# Wallet algorithm
In the following we describe a wallet algorithm which allows making any micropayment in the event where the user doesn't care about the excess mass or if a mutual transaction cannot be created. In most cases, the current wallet implementations require no modifications. Here we present an iterative algorithm for handling the cases which do require adaptations. Notably, the algorithm requires using the [relaxed](#relaxed-storage-mass-formula) formula and wouldn't converge without this relaxation. 

Let `M` be the maximal mass per transaction and denote `F` to be the standard fee required for a transaction with mass `M`. Assume the user desires to make a payment with `payment` value and that we already collected sufficient inputs such that `sum(inputs) >= payment + F`. Furthermore, assume that  `payment >> F` (otherwise it might not be worth making this payment altogether). The algorithm assumes that a transaction composed of `inputs` and `outputs = [payment, sum(inputs) - payment - F]` has `compute_mass <= M` but `storage_mass > M`. Otherwise if `compute_mass` is too large, it should be solved using the current methods of compounding by building a transaction tree. Eventually we arrive at a root transaction where `compute_mass` is low enough, at which point `storage_mass` can be dealt with as follows: 

```python 
def build_txs(inputs, payment):
    txs = []
    while storage_mass(inputs, [payment, sum(inputs) - payment - F]) > M:
        out = sum(inputs) - F
        neg = negative_mass(inputs, 2) 
        # Requires solving a simple quadratic equation over the variable x
        alpha = x between 0.0 and 1.0 such that C/(x * out) + C/((1-x) * out) - neg == M
        outputs = [alpha * out, (1-alpha) * out]
...
def storage_mass(inputs, outputs):
    # returns the storage mass as described by the relaxed formula
    ...

def negative_mass(inputs, num_outputs):
    # returns the negative component of the storage mass as described by the relaxed formula
    ...
```

**Remarks:**
- Without the relaxation which uses the harmonic mean over the inputs, the loop would not converge due to the averaging done over the inputs, yielding the same `neg` value over and over
- In all intermediate transactions built by `build_txs`, the recipient for both outputs should be the change address of the *sender* 
- In the final transaction built, it is possible that the `fee` can be decreased below `F`
