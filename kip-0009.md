...
### Constants
We suggest setting $C=10^{12}$. Note that transaction input and output values are represented in dworks (currently known in code as sompis), which are the smallest unit of KAS value (a single KAS is equal $10^{8}$ dworks).  
