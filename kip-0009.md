...
We call this the *relaxed formula*, as it treats inputs and outputs the same. With some work, one can see that the relaxed formula cannot substitute the standard formula. However, we show in [3] that the quadratic bound still holds when using `storage_mass*` for transactions satisfying $|I|\le|O|\le 2$.
...
I DON'T UNDERSTAND THIS SECTION WELL ENOUGH TO REWRITE IT
