...
We model a transaction of two sets of values, the input values $I$ and output values $O$ (strictly speaking $I$ and $O$ are multisets, as the same value can occur several times, but we ignore this technicality here). We use the notation $x^+ = \max\lbrace x,0 \rbrace$. The storage mass is defined as follows:
...
In contrast, one can check that $\text{compute mass}(\text{tx}) \ge 2000$, so we see that the total mass is not changed, despite the fact that `txn` is a relatively low-value transaction with more outputs than inputs.

More generally, we see that the `storage_mass` becomes dominant only when relatively small inputs emerge (where the exact threshold is inversely proportional to the total budget of the transaction). Still, even in everyday use, we see that small outputs can emerge. Small UTXOs commonly appear as change or as micropayments, and however we set $C$, we should account for the possibility that a standard payment from a standard wallet could exceed this threshold, affecting the QoS of everyday users. In section [Wallet algorithm](#wallet-algorithm) we will show how to compose transactions for sending arbitrarily small values (even below $0.1$ `KAS`), and in section [Micropayments](#micropayments) we will discuss strategies for mitigating this cost altogether.


### Compounding transactions
A transaction `txn` is *compounding* if $|O| \le |I|$ and all values in $O$ are the same, namely $\text{budget}/|O|$ (most commonly, we have that $|O|=1$). Since all values in $O$ are the same we have that $H(O) = A(O) = \text{budget}/|O|$ and we get that
$$\text{storage mass}(\text{txn}) = C\cdot\left(\frac{|O|}{A(O)} - \frac{|I|}{A(I)}\right)^+ = C\cdot\left(\frac{|O|^2 - |I|^2}{\text{budget}}\right)^+ = 0\text{.}$$
...
$$\text{storage mass}\left(\text{txn}\right)=C\cdot\left(\frac{\left|O\right|^{2}}{\text{budget}-\text{fee}}-\frac{\left|I\right|^{2}}{\text{budget}}\right)$$
...
Consider a transaction `txn` with a single input of value $V$ and two outputs of values $v$ and $V-v$ where $v\ll V$, then it follows that
$$\text{storage mass}\left(\text{txn}\right)\approx C\cdot\left(\frac{1}{V-v}+\frac{1}{v}-\frac{1}{V}\right)\approx\frac{C}{v}\text{.}$$
...
### Wallet algorithm
In this section we provide an optimal algorithm for making a payment of a given value using a given set of UTXOs. The algorithm is "optimal" in the sense that it minimizes the storage mass. In scenarios where the storage mass is lower than the computation mass, the current algorithm should be used instead.

We assume that there is a maximal storage mass $M$ a single transaction could have. The value $M$ can be either the common mempool bound (which is currently set to 100,000 grams), or user specified. We also assume there is a conversion ratio $r$ of mass to fee. The value of $r$ is usually obtained by the wallet software by monitoring the network usage. Since the block capacity is measured in grams, the assumption that the fee is linearly proportional to the mass makes sense (barring the unrealistic scenarios where almost all transactions typically admit large masses, say in the tens of kilograms). Hence a transaction of mass $m$ will pay a fee of $rm$. We let $F=rM$ denote the fee of a maximally heavy transaction.
