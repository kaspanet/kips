...
This creates an "additive-only spending" pattern where UTXOs can only be spent by increasing their value, effectively preventing value extraction without owner authorization.
...
2. Make payouts even with a single coinbase input by "borrowing" participant UTXOs
...
2. *Authorized Borrower Spending*: Someone can spend the UTXO only if they:
   1. Know a pre-defined secret (implemented as a keypair)
