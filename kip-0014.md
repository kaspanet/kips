...
Additionally, the hardfork introduces critical storage regulation with KIP-9, which implements a storage mass formula to address UTXO set growth challenges and ensure resilience against state bloat in various conditions. This is complemented by KIP-13, which regulates transient storage to address short-term demands. Another major component in this hardfork is KIP-10, which introduces new introspection opcodes to Kaspa's script engine. Through such introspection, these opcodes enable the concept of covenants, which allow for advanced transaction controls, including the design of additive addresses that support microtransactions and complement KIP-9.
...
8. **Enabling transaction payloads**:
   - TODO

9. **Activation of additional KIPs**:
