 ...
 * Each posterity header contains a pointer to the previous posterity header. Hence, the chain of posterity headers is verifiable all the way down to genesis. In particular, obtaining and verifying the posterity chain is part of the process of syncing a new node.
...
In theory, we could have defined our result such that the new validation step only applies to posterity blocks. However, this makes posterity blocks harder to construct and verify than other blocks. This might significantly increase their propagation time, decentivize miners from mining them, or have other unexpected adverse effects. To avoid this, we propose requiring all blocks to pass this verification step.
...
Increasing posterity density to once per hour (that is, setting N=3600), decreases the largest PoChM size to 6 kilobytes.

If network conditions improve so much that the selected chain grows at a rate of 10 blocks per second, the largest PoChM size would be 11 kilobytes for 24 hour density and 8 kilobytes for one hour density.

# Resource Cost

24 hours poseterity density:
 ...
 * Block Validation: the number of hashes/second will reduce to 32 hashes/second for 1 block/second selected chain growth, or to 300 hashes/second for 10 blocks/second selected chain growth.
